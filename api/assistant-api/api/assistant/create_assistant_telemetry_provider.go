...
func (assistantApi *assistantGrpcApi) CreateAssistantTelemetryProvider(
	ctx context.Context,
	req *protos.CreateAssistantTelemetryProviderRequest,
	...
	provider, err := assistantApi.assistantTelemetryService.Create(
		ctx,
		iAuth,
		req.GetAssistantId(),
		req.GetProviderType(),
	...
	if err != nil {
		return exceptions.BadRequestError[protos.GetAssistantTelemetryProviderResponse](
			"Unable to create assistant telemetry provider.",
		)
	}
