...
import (
	...
	plugins_provider_calendar_calcom "github.com/rapidaai/pkg/plugins/providers/calendar/calcom"
	plugins_provider_calendar_noop "github.com/rapidaai/pkg/plugins/providers/calendar/noop"
...
// Provider is the extension contract for calendar providers.
// Implement this in any package and register it via RegisterProvider
// or domains.WithCalendarProviders(...).
type Provider interface {
	Code() string
	BookMeeting(ctx context.Context, input map[string]interface{}, credential map[string]interface{}, logger commons.Logger) (map[string]interface{}, error)
}
...
func NewPlugin(customProviders ...Provider) *Plugin {
	...
	p.RegisterProvider(plugins_provider_calendar_noop.New())
	p.RegisterProvider(plugins_provider_calendar_calcom.New())
...
func (p *Plugin) Execute(ctx context.Context, req plugins_types.ExecuteRequest, deps plugins_types.ExecuteDeps) (*plugins_types.Result, error) {
	...
	if operation != "book_meeting" {
		return plugins_types.Failure(provider, operation, "unsupported calendar operation", nil), nil
	}
	...
	if !ok {
		return plugins_types.Failure(provider, operation, "calendar provider not found", nil), nil
	}

	result, err := client.BookMeeting(ctx, req.Input, vaultCredential.GetValue().AsMap(), deps.Logger)
	if err != nil {
