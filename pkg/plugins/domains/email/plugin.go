...
import (
	...
	plugins_provider_email_noop "github.com/rapidaai/pkg/plugins/providers/email/noop"
	plugins_provider_email_sendgrid "github.com/rapidaai/pkg/plugins/providers/email/sendgrid"
...
// Provider is the extension contract for email providers.
// Implement this in any package and register it via RegisterProvider
// or domains.WithEmailProviders(...).
...
func NewPlugin(customProviders ...Provider) *Plugin {
	...
	p.RegisterProvider(plugins_provider_email_noop.New())
	p.RegisterProvider(plugins_provider_email_sendgrid.New())
...
func (p *Plugin) Execute(ctx context.Context, req plugins_types.ExecuteRequest, deps plugins_types.ExecuteDeps) (*plugins_types.Result, error) {
	...
	if operation != "send_email" {
		return plugins_types.Failure(provider, operation, "unsupported email operation", nil), nil
	}
	...
	if !ok {
		return plugins_types.Failure(provider, operation, "email provider not found", nil), nil
	}
